...
import { validateArray, validateRequired, validateString } from '../utils/validators.js';
...
const DEFAULT_ACCEPTANCE_MODEL = 'gemini-3.1-pro-preview';
...
function normalizeFinding(finding, fallbackTitle) {
    return {
...
async function readExplicitFiles(filePaths) {
...
async function collectReviewFiles(params) {
    ...
    if (providedInputCount === 0) {
        throw new Error('One input source is required: filePath, content, directory, filePaths, or files.');
    }
    if (providedInputCount > 1) {
        throw new Error('Provide exactly one input source. Do not mix filePath, content, directory, filePaths, and files in one request.');
    }
    if (params.filePath) {
...
function buildAcceptancePrompt(params, files, strictness) {
    const lines = [
    ...
    lines.push('## Review Mode', `- Strictness: ${strictness}`, strictness === 'strict'
    ...
    lines.push('## Files Under Review', `- Total files: ${files.length}`, '');
    for (const file of files) {
        lines.push(`### ${file.path}`, '```', file.content, '```', '');
    }
    lines.push('## Return JSON', '{', '  "verdict": "pass" | "needs_work" | "fail",', '  "summary": "Short review summary",', '  "blockingFindings": [', '    {', '      "title": "Blocking issue title",', '      "severity": "high" | "medium" | "low",', '      "description": "Why this blocks acceptance",', '      "location": "Optional file path or line reference",', '      "suggestion": "Concrete fix suggestion"', '    }', '  ],', '  "nonBlockingFindings": [', '    {', '      "title": "Non-blocking issue title",', '      "severity": "high" | "medium" | "low",', '      "description": "Why it matters",', '      "location": "Optional file path or line reference",', '      "suggestion": "Concrete fix suggestion"', '    }', '  ],', '  "coverageGaps": ["Any area you could not confidently assess"],', '  "recommendedNextSteps": ["Ordered next steps for the coding agent"]', '}');
    return lines.join('\n');
}
export async function handleAcceptanceAssist(params, apiKey) {
    try {
        ...
        if (params.strictness && !ACCEPTANCE_STRICTNESS.includes(params.strictness)) {
            throw new Error(`Invalid strictness: ${params.strictness}. Must be one of: ${ACCEPTANCE_STRICTNESS.join(', ')}`);
        }
        ...
        const strictness = params.strictness || 'standard';
        ...
        if (bytesAnalyzed > MAX_ACCEPTANCE_INPUT_BYTES) {
            throw new Error(`Acceptance input exceeds ${MAX_ACCEPTANCE_INPUT_BYTES} bytes. Narrow the scope or review fewer files at once.`);
        }
        const thinkingLevel = strictness === 'strict' || files.length > 1 || targetType === 'codebase' ? 'high' : 'low';
...
//# sourceMappingURL=acceptance-assist.js.map