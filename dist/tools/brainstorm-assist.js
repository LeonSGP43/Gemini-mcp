...
import { validateArray, validateNumber, validateRequired, validateString } from '../utils/validators.js';
...
async function loadContextFiles(params) {
    ...
    for (const filePath of requestedPaths) {
        ...
        if (totalBytes > MAX_CONTEXT_BYTES) {
            throw new Error(`Context files exceed ${MAX_CONTEXT_BYTES} bytes. Narrow the context files or trim their content.`);
        }
...
function buildBrainstormPrompt(params, mode, count, contextSnippet) {
    const lines = [
    ...
    lines.push('## Output Requirements', `- Generate exactly ${count} candidate directions`, `- Mode: ${mode}`, '- Favor technically grounded options over generic strategy talk', '- Recommend one best direction and explain why', '');
    switch (mode) {
        case 'explore':
            lines.push('## Mode Guidance', '- Maximize diversity of approaches', '- Cover different technical and product angles', '- Make tradeoffs explicit', '');
            break;
        case 'refine':
            lines.push('## Mode Guidance', '- Focus on the strongest few approaches', '- Compare them directly on cost, speed, and risk', '- Bias toward clear recommendation quality', '');
            break;
        case 'ship':
            lines.push('## Mode Guidance', '- Prioritize execution-ready ideas', '- Prefer smaller, testable changes and rollout steps', '- Call out what can be done next in code', '');
            break;
    }
    lines.push('## Return JSON', '{', '  "summary": "Short synthesis of the brainstorm",', '  "recommendedDirection": "Single recommended direction",', '  "ideas": [', '    {', '      "title": "Idea title",', '      "rationale": "Why this idea matters",', '      "benefits": ["benefit 1"],', '      "risks": ["risk 1"],', '      "implementationOutline": ["step 1", "step 2"]', '    }', '  ],', '  "nextSteps": ["next step 1", "next step 2"]', '}');
    return lines.join('\n');
}
export async function handleBrainstormAssist(params, apiKey) {
    try {
        ...
        const thinkingLevel = mode === 'explore' ? 'low' : 'high';
...
//# sourceMappingURL=brainstorm-assist.js.map