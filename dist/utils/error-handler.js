...
export function handleAPIError(error) {
    ...
    if (error instanceof SecurityError || message.includes('Security validation failed')) {
        return createMCPError(ERROR_CODES.INVALID_PARAMS, message, {
    ...
    if (/file not found|directory not found|no permission to access file|path is a directory|cannot read binary file/i.test(message)) {
        return createMCPError(ERROR_CODES.INVALID_PARAMS, message, { originalError: message });
    }
    // API Key error
    if (message.includes('API key') || message.includes('Invalid key')) {
        return createMCPError(ERROR_CODES.API_ERROR, 'Invalid API key. Please check your GEMINI_API_KEY environment variable.', { originalError: message });
    }
    // Quota error
    if (message.includes('quota') || message.includes('rate limit')) {
        return createMCPError(ERROR_CODES.RATE_LIMIT, 'API quota exceeded or rate limit reached. Please try again later.', { originalError: message });
    }
    // Timeout error
    if (message.includes('timeout')) {
        return createMCPError(ERROR_CODES.TIMEOUT, 'Request timeout. The operation took too long to complete.', { originalError: message });
    }
    // Network / transport errors
    if (/fetch failed|network|econnreset|econnrefused|enotfound|eai_again|proxy/i.test(message)) {
        return createMCPError(ERROR_CODES.API_ERROR, 'Network request to Gemini failed. Check proxy, connectivity, or API availability.', { originalError: message });
    }
    // Model not supported
    if (/\bmodel\b/i.test(message)
        && /not supported|not available|unsupported|not found/i.test(message)) {
        return createMCPError(ERROR_CODES.MODEL_NOT_SUPPORTED, 'The specified model is not supported or not available.', { originalError: message });
    }
    // Generic API error
    return createMCPError(ERROR_CODES.API_ERROR, message || 'An error occurred while calling the Gemini API.', { originalError: message });
}
/**
