...
export interface FileContent {
  /** Relative path (relative to directory parameter or current directory) */
  path: string;
  /** Absolute path */
  absolutePath: string;
  /** File content */
...
export interface ReadDirectoryOptions {
  /** glob include patterns, default ["**\/*"] */
  include?: string[];
  /** glob exclude patterns, default excludes common ignored directories */
  exclude?: string[];
...
export async function readFile(
  ...
  try {
    ...
    if (nodeError.code === 'EACCES') {
      throw new FileReadError(
        `No permission to access file: "${filePath}"`,
        filePath,
        nodeError
      );
    }
